...
bool Zdrive_IsOurs(const CAN_RxHeaderTypeDef *Rxheader, uint8_t bus)
{
    ...
    /*
     * 操作码(高 4 位)必须 >= SES(0x01)。
     *
     * 主板动作命令 StdId = 0x01..0x08，
     * 高 4 位恰好是 0x0、低 4 位也落在 1..N 范围内，
     * 如果只看电机号(低 4 位)会把命令帧误判成 ZDrive 反馈，
     * 从而误触发"反馈就绪 -> Begin 置 true"。
     *
     * 这里直接把 opcode == 0 的帧排除。
     */
    if ((uint32_t)(Rxheader->StdId >> 4U) < (uint32_t)SES)
    {
        return false;
    }
...
static void Zdrive_RunMachine(Zdrive *motor, uint8_t id)
{
    ...
    case Zdrive_Speed:
         ...
         * 速度目标发生变化时才发送一次。
         */
        if (fabs(motor->valSetNow.speed_rpm -
                 motor->valSetPre.speed_rpm) > 0.1f)
        {
            ZdriveSet(
                motor->valSetNow.speed_rpm,
                id,
                VelIn
            );

            motor->valSetPre.speed_rpm =
                motor->valSetNow.speed_rpm;
        }
    ...
    case Zdrive_Current:
        ...
        // ZdriveSetMIT(
        //     motor->valSetNow.speed_rpm,
        //     motor->valSetNow.pos_deg,
        //     motor->valSetNow.current_A,
        //     motor->param.kpPos,
        //     motor->param.kpVel,
        //     id
        // );
    ...
    case Zdrive_Postion:
        ...
        if (!motor->pvtparam.PVTflag)
        {
             ...
             *          ZdriveSet(0°);
             *
             * 这个逻辑会在复位过程中不断向 FIFO 塞入 0°，
             * 最终可能导致 FIFO 堵塞。
             *
             * 所以这里取消 0° 特殊重发。
             */

            if (fabs(motor->valSetNow.pos_deg -
                     motor->valSetPre.pos_deg) > 0.01f)
            {
                /*
                 * 先保存新的目标位置。
                 */
                motor->valSetPre.pos_deg =
                    motor->valSetNow.pos_deg;
